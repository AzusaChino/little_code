...
class Trie {
    ...
    // Runs a DFS on trie starting with given prefix and adds all the words in
    // the result, limiting result size to 3
...
private:
    vector<vector<string>> _suggestedProducts(vector<string> &products,
                                              string          searchWord) {
        ...
        for (char &c : searchWord) {
            ...
            start = lower_bound(products.begin() + bsStart, products.end(),
                                prefix) -
                    products.begin();
            ...
            // Loop runs until `i` reaches the end of input or 3 times or till
            // the prefix is same for `products[i]` Whichever comes first.
            for (int i = start;
                 i < min(start + 3, n) &&
