...
class UnionFind:
    ...
    def add(self, u):
        if self.isExist(u):
            return  # Only add if not existed yet!
        self.parent[u] = u
        self.size[u] = 1
    ...
    def union(self, u, v):
        ...
        ):  # Merge the smaller component to the bigger component
    ...
    def largestIsland(self, grid: List[List[int]]) -> int:
        ...
        def landNeighbors(r, c):
            for i in range(4):
                ...
                neiId = nr * n + nc
                if nr < 0 or nr == m or nc < 0 or nc == n or not uf.isExist(neiId):
                    continue
                yield neiId
        ...
        for r in range(m):
            for c in range(n):
                ...
                sizeFormed = (
                    1  # Start with 1, which is matrix[r][c] when turning from 0 into 1
                )
                for p in neiParents:
