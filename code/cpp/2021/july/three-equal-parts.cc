...
public:
    vector<int> threeEqualParts(vector<int>& A) {
        ...
        for (int c : A)
            if (c == 1) countNumberOfOnes++;

        // If no 1 is found, that means we can return ans as {0, size-1}
        if (countNumberOfOnes == 0) return {0, A.size() - 1};

        // If no of 1's is not a multiple of 3, then we can never find a
        // possible partition since there will be atkeast one partition that
        // will have different no of 1's and hence different binary
        // representation For example, Given : 0000110  110  110
        //       |  |    |
        //       i       j
        // Total no of ones = 6
        // 0000110         110      110
        //     |           |        |
        //     start       mid      end
        // Match starting from first 1, and putting 2 more pointers by skipping
        // k 1's
        ...
        if (end == A.size()) return {start - 1, mid};
