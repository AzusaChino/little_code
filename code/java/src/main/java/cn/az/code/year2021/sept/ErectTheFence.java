...
public class ErectTheFence {
    ...
    public int[][] outerTrees(int[][] trees) {
        ...
        for (int i = 2; i < n; i++) {
            int[] top = stack.pop();
            while (compAngle(stack.peek(), top, trees[i]) < 0) top =
                stack.pop();
            stack.push(top);
            stack.push(trees[i]);
        }
    ...
    int[] bottomLeft(int[][] trees) {
        ...
        for (int[] tree : trees) {
            if (
                bottomLeft[1] > tree[1] ||
                (bottomLeft[1] == tree[1] && bottomLeft[0] > tree[1])
            ) bottomLeft = tree;
        }
        return bottomLeft;
    }
    ...
    void sort(int[][] trees, int[] ref) {
        ...
        while (i >= 0 && compAngle(trees[i], ref, q) == 0) i--;
